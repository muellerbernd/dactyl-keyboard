...
shape_config = {
    ...
    "alpha": pi / 18.0,  # curvature of the columns
    "beta": pi / 36.0,  # curvature of the rows
    "centercol": 3,  # controls left_right tilt / tenting (higher number is more tenting)
    "centerrow_offset": 2,  # rows from max, controls front_back tilt
    # 'tenting_angle':  pi / 12.0,  # or, change this for more precise tenting control
    "tenting_angle": pi / 32.0,  # or, change this for more precise tenting control
    ...
    "thumb_offsets": [6, -3, 2],
    "keyboard_z_offset": (
        16  # controls overall height# original=9 with centercol=3# use 16 for centercol=2
    ),
    ...
    "default_thumb_screw_xy_locations": [[-21, -55]],
    ...
    "minidox_thumb_screw_xy_locations": [[-37, -30]],
    ...
    "tbiw_oled_center_row": 0.75,  # not none, offsets are from this position
...
def save_config():
    ...
    if got_opts:
        with open(
            os.path.join(r"..", "configs", shape_config["config_name"] + ".json"),
            mode="w",
        ) as fid:
